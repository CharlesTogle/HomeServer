...
function buildPath(folderId: string): FolderRecord[] {
  ...
  while (currentFolder !== null) {
    ...
    currentFolder =
      parentId === null
        ? null
        : (folders.find((entry) => entry.id === parentId) ?? null);
  }
...
export async function deleteItem(input: DeleteItemInput): Promise<void> {
  ...
  if (hasChildFolders || hasChildFiles) {
    throw new Error("Empty the folder before deleting it.");
  }
...
export async function moveItem(input: MoveItemInput): Promise<void> {
  ...
  if (
    destinationFolder.id === folder.id ||
    descendantIds.has(destinationFolder.id)
  ) {
    throw new Error(
  ...
  const duplicateFolder = folders.some(
    (entry) =>
      ...
      entry.name.localeCompare(folder.name, undefined, {
        sensitivity: "base",
      }) === 0,
  );
