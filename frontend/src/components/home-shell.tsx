...
export function HomeShell(): React.JSX.Element {
  ...
  const { reset, selectedFileId, selectedFolderId, setSelectedFileId, setSelectedFolderId } =
    useWorkspaceStore(
      useShallow((state) => ({
        reset: state.reset,
  ...
  async function handleRequestDownload(kind: LibraryItemKind, id: string): Promise<void> {
    resetActionErrors()
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        description={
          deleteTarget?.kind === 'folder'
            ? 'Only empty folders can be removed in this preview.'
            : 'This removes the file from the current preview session.'
        }
      ...
      <ConfirmationModal
        ...
        title="Sign out?"
        description="This clears the current session from memory and resets the workspace."
